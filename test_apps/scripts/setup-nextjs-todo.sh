#!/bin/bash
# setup-nextjs-todo.sh - Setup the Next.js Todo List example
...
# Check if app already exists (we have a local template)
if [[ ! -d "$APP_DIR" ]]; then
    # Clone the example (slow - downloads from GitHub)
    clone_supabase_example "examples/todo-list/nextjs-todo-list" "$APP_NAME"
else
